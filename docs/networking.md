...
- macOS: vmnet.framework (kernel-level NAT/bridge) and user-mode (SLIRP) networking
...
| Bridge mode | Supported | No | Supported | No |
...
vmnet mode (requires sudo, best performance):
```bash
...
# vmnet provides DHCP
ip link set eth0 up
udhcpc -i eth0              # Obtain IP via DHCP
...
### Short-term
- [ ] CLI options for vmnet host/bridged modes (`-n vmnet:host`, `-n vmnet:bridged=en0`)
- [ ] Performance benchmarking across backends
- [ ] Network throughput tests in CI

### Long-term
- [ ] Support for custom IP ranges (vmnet shared mode)
- [ ] MAC address customization
- [ ] Multiple network interfaces
- [ ] IPv6 support
- [ ] Port forwarding configuration for user mode
